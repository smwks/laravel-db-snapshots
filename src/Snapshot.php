...
class Snapshot
{
    ...
    {
        $smartCache = config('db-snapshots.cache_by_default', false);
    ...
    public function load($useLocalCopy = false, $keepLocalCopy = false, $forceDownload = false): array
    {
        ...
        $dumpFile = $this->snapshotPlan->localDisk->path("{$this->snapshotPlan->localPath}/{$this->fileName}");

        $driver = $this->snapshotPlan->getDriver();
        $dbConfig = $this->snapshotPlan->getDatabaseConnectionConfig();

        $zcatUtil = config('db-snapshots.utilities.zcat');

        $loadCommand = $driver->buildLoadCommand($dumpFile, $dbConfig['database']);

        $this->snapshotPlan->runCommandWithCredentials(
            "$zcatUtil $dumpFile | $loadCommand"
        );
